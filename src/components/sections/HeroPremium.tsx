  ...
  return (
    <section className="relative w-full px-4 pt-24 pb-20 md:pt-32 md:pb-32 overflow-hidden flex flex-col items-center min-h-[100svh] bg-transparent isolate z-0">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden z-0 hidden lg:block">
        ...
        {/* 🚨 AJUSTEMENT ICI : Descendu de top-[18%] à top-[25%] */}
        <div className="absolute top-[25%] left-[6%] anim-float-fast">
          <div className="flex flex-col gap-2 p-4 rounded-2xl bg-white/40 backdrop-blur-md border border-white/60 shadow-xl rotate-[-15deg]">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-brand-primary/10 rounded-lg">
                <Code2 className="w-5 h-5 text-brand-primary" />
              </div>
        ...
        <div className="absolute bottom-[43%] right-[10%] anim-float-slow">
          <div className="flex flex-col gap-3 p-4 rounded-2xl bg-white/40 backdrop-blur-md border border-white/60 shadow-xl rotate-[15deg]">
            <div className="flex items-center gap-3 mb-1">
              <div className="p-2 bg-brand-accent/10 rounded-lg">
                <TrendingUp className="w-5 h-5 text-brand-accent" />
              </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center w-full max-w-6xl relative z-10 pointer-events-none px-2">
        ...
        <div className="relative mb-6 md:mb-10 w-full flex justify-center">
          ...
          <h1 className={cn("max-w-5xl text-4xl md:text-6xl lg:text-[4.5rem] font-heading font-extrabold text-text-main tracking-tighter leading-[1.1] md:leading-[1.05] opacity-0", mounted && "animate-fade-in-up-blur")} style={getAnimationDelay(2)}>
            ...
            <span className="relative inline-flex flex-col h-[1.1em] overflow-hidden align-top">
              <span className={cn("block text-transparent bg-clip-text bg-gradient-to-r from-brand-primary via-brand-accent to-brand-primary selection:text-brand-primary drop-shadow-sm animate-text-gradient transition-all duration-500 ease-in-out", isAnimatingWord ? "opacity-0 translate-y-8" : "opacity-100 translate-y-0")}>
        ...
        <p className={cn("max-w-3xl text-lg md:text-xl lg:text-2xl text-text-muted mb-10 md:mb-12 font-light leading-relaxed opacity-0 mt-4 md:mt-2", mounted && "animate-fade-in-up-blur")} style={getAnimationDelay(3)}>
        ...
        <div className={cn("pointer-events-auto flex flex-col sm:flex-row items-center justify-center gap-4 w-full sm:w-auto opacity-0", mounted && "animate-fade-in-up-blur")} style={getAnimationDelay(4)}>
          ...
          <Button variant="ghost" size="lg" className="w-full sm:w-auto text-text-main border border-gray-200 bg-white/50 backdrop-blur-md active:bg-gray-50 active:scale-95 transition-all duration-300" asChild>
            <Link href="/services">Voir nos domaines d'expertise</Link>
          </Button>
        </div>
      </div>

      <div className={cn("relative flex flex-col items-center gap-2 md:gap-3 opacity-0 pointer-events-none mt-12 shrink-0 z-10", mounted && "animate-fade-in-up-blur")} style={getAnimationDelay(6)}>
        <span className="text-[9px] md:text-[10px] uppercase tracking-[0.3em] text-text-muted/60 font-medium">Découvrir</span>
      ...
      <div className="absolute bottom-0 left-0 w-full overflow-hidden leading-[0] transform translate-y-[1px] z-20 pointer-events-none">
        <svg className="relative block w-full h-[40px] md:h-[80px] lg:h-[100px]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 100" preserveAspectRatio="none">
          <path className="fill-white" d="M0,50 C360,150 1080,-50 1440,50 L1440,100 L0,100 Z"></path>
        </svg>
      </div>
    </section>
  );
